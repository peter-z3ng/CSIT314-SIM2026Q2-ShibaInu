...
export function GenerateDailyReportPage({
  account,
  dailyReport,
}: {
  ...
  const displayDailyReport = () => (
    <section className="mx-auto max-w-7xl px-5 py-8 lg:px-8">
      <div className="flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <h1 className="mt-2 text-4xl font-bold">Daily Reports</h1>
        </div>

        <div className="flex flex-wrap items-center gap-3">
          <ReportPeriodToggle profilePath={profileToPath(account.profile)} active="daily" />
          <button
            type="button"
        ...
        <table className="w-full min-w-[900px] border-collapse">
          <thead>
            <tr className="border-b border-[#f0d8bd] text-sm text-[#6f6258]">
              <th className="px-6 py-5 text-center font-bold">Day</th>
...
function InsightCard({
  ...
  return (
    <article className="rounded-4xl border border-[#FFB347] bg-white/40 p-5 shadow-lg hover:scale-110 transition">
...
function downloadCSV(reports: Report[]) {
  ...
  link.href = url;
  link.download = "daily-user-report.csv";
  link.click();
