...
export function GenerateMonthlyReportPage({
  account,
  monthlyReport,
}: {
  ...
  const displayMonthlyReport = () => (
    <section className="mx-auto max-w-7xl px-5 py-8 lg:px-8">
      <div className="flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <h1 className="mt-2 text-4xl font-bold">Monthly Reports</h1>
        </div>

        <div className="flex flex-wrap items-center gap-3">
          <ReportPeriodToggle profilePath={profileToPath(account.profile)} active="monthly" />
          <button
            type="button"
...
function downloadCSV(reports: Report[]) {
  ...
  link.href = url;
  link.download = "monthly-user-report.csv";
  link.click();
