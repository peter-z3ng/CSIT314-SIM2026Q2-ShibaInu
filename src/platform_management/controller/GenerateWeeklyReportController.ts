...
export class GenerateWeeklyReportController {
  // getWeeklyReport()
  async getWeeklyReport(): Promise<Report[]> {
    ...
    const report = buildWeeklyPeriods().map((period) => {
    ...
    return UserAccount.getWeeklyReport(report);
  }
}
...
function buildWeeklyPeriods() {
  ...
  return Array.from({ length: count }, (_, index) => {
    ...
    const start = startOfDay(addDays(now, -offset * 7 - 6));
